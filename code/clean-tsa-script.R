...
tsa_proj <- vec_files %>% 
  map(get_tsa_df, dshs_df = dshs_df, example_tsa = vec_files[1]) %>% 
  bind_rows()
...
plot_individual_tsa_variable("Rt", expression(paste("Effective reproduction number ", italic("R(t)"))), tsa_df, dshs_df, retrospective_days = 120)
plot_individual_tsa_variable("total_new_H","COVID-19 Hospital Admissions", tsa_df, dshs_df, retrospective_days = 120)
plot_individual_tsa_variable("total_H","COVID-19 Hospitalizations", tsa_df, dshs_df, retrospective_days = 120)
plot_individual_tsa_variable("icu","COVID-19 ICU Patients", tsa_df, dshs_df, retrospective_days = 120)
...
tsa_proj %>% 
  ...
  arrange(tsa_letter) %>% 
  write_csv(paste0("produced_figures/tsa_proj-", Sys.Date(), "/tsa-table1.csv"))
...
tsa_proj %>% 
  ...
  arrange(tsa_letter) %>% 
  write_csv(paste0("produced_figures/tsa_proj-", Sys.Date(), "/tsa-table2.csv"))
